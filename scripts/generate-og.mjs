...
/** The hosted wordmark's canvas. See EMAIL_LOGOS below for why they exist. */
...
/*
 * The wordmark, hosted, for the Google Workspace append footer and the email
 * signature. That setting takes no HTML and no uploaded file: an image can only
 * come from a public URL, and Drive is not allowed, so the site serves it. They
 * ship in public/ for that reason and the pages do not use them.
 *
 * Sized at twice the roughly 210px a footer shows, so they stay sharp on a
 * retina screen. The canvas matches the lockup's own proportion, so there is no
 * dead space around it to read as a misaligned box next to the footer text.
 *
 * Two grounds because the wordmark is inked in `ink`. The transparent one takes
 * whatever the client paints, which is right until the client paints it dark;
 * it stays for the append footer, whose image control has no ground of ours to
 * match. The paper one holds in either mode and is what the signature uses.
 */
const EMAIL_LOGOS = [
  { file: 'email-logo.png', ground: null },
  { file: 'email-logo-paper.png', ground: 'paper' },
];
...
for (const { file, ground } of EMAIL_LOGOS) {
  const emailLogo = await fitInto(lockup.data, {
    ...EMAIL_LOGO_SIZE,
    ground: ground ? palette[ground] : null,
    margin: 0,
  });
  await writeFile(join(publicDir, file), emailLogo);
  console.log(`${file} generated (${ground ?? 'transparent'}, ${emailLogo.byteLength} bytes)`);
}
